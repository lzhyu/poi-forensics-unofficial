...
def contrastive_loss(features: torch.Tensor,
    ...
    # get labels
    # mask should be B X B M[i][j] = (i.id==j.id)
    ...
    # B X B X feature_dim
    # features[i] = [features], features[i][j] = features_j
    ...
    logits = (
        logits - torch.max(logits.detach(), dim=0, keepdim=True))
    ...
    #FIXME: since we have view=1, mask should not be changed
    # self_diagonal_mask: B X B diagonal
    self_diagonal_mask = torch.eye(batch_size)
    ...
    assert(num_positives_per_row>=1)
    # reduce sum
